...
## Scripts are here
URL="https://dove.celenity.dev/upgrade_scripts"
...
case ${DISTRO} in
	...
	"arch" | "Arch" | "ARCH")
		...
		case ${HELPER} in
			"paru" | "Paru" | "PARU")
				upgrade_dove ${URL}/${SCRIPT[0]} ${SCRIPT[0]}
				;;
			"yay" | "Yay" | "YAY")
				upgrade_dove ${URL}/${SCRIPT[1]} ${SCRIPT[1]}
				;;
		esac
		;;

	"fedora"| "Fedora" | "FEDORA")
		upgrade_dove ${URL}/${SCRIPT[3]} ${SCRIPT[3]}
		;;

	"macOS" | "macos" | "MacOS" | "MACOS")
		upgrade_dove ${URL}/${SCRIPT[4]} ${SCRIPT[4]}
		;;
esac
