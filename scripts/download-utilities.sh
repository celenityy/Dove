...
function download() {
  ...
  local -r file_name=$("${DOVE_BASENAME}" "${output_file}")
  ...
  "${DOVE_CURL}" ${DOVE_CURL_FLAGS} --location "${url}" --output "${output_file}" || local download_failed=1
...
function clone_git_repo() {
  ...
  if [[ "${revision}" == 'no-revision' ]]; then
    if [[ "${submodules}" == 1 ]]; then
      "${DOVE_GIT}" clone "${url}" "${path}" --depth="${depth}" --recurse-submodules || local clone_failed=1
    else
      "${DOVE_GIT}" clone "${url}" "${path}" --depth="${depth}" || local clone_failed=1
    fi
  else
    if [[ "${submodules}" == 1 ]]; then
      "${DOVE_GIT}" clone "${url}" "${path}" --depth="${depth}" --revision="${revision}" --recurse-submodules || local clone_failed=1
    else
      "${DOVE_GIT}" clone "${url}" "${path}" --depth="${depth}" --revision="${revision}" || local clone_failed=1
    fi
  fi
