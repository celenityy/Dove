#!/bin/bash

set -euo pipefail

# This is a basic script used to create the .zip files you see in the 'archives' directory.
# We could just clone the entire source code - though lots of of it are completely unnecessary for packaging.
# This creates a slim .zip file only containing what we actually need.

# Script should be ran from inside the directory where you store Dove, not directly from the 'archives' or `build` folder...

# Set-up our environment
bash -x $(dirname $0)/env.sh || error_fn
echo
source $(dirname $0)/env.sh || error_fn
echo

rm -rf "${DOVE_ARCHIVES}/*" || error_fn
echo

# dot_clean for OS X
DOVE_DOT_CLEAN='/usr/sbin/dot_clean -mv'

# zip
DOVE_ZIP='zip -r -FS'

if [ "${DOVE_LINUX}" == 1 ]; then
    if [[ "${DOVE_OS}" == 'osx' ]]; then
        ${DOVE_DOT_CLEAN} "${DOVE_LINUX_OUTPUTS}" || error_fn
        echo
    fi

    pushd "${DOVE_LINUX_OUTPUTS}" || error_fn
    echo
    echo_green_text "Creating ${DOVE_ARCHIVES}/dove-linux.zip..."
    ${DOVE_ZIP} "${DOVE_ARCHIVES}/dove-linux.zip" * -x 'unused/*' || error_fn
    echo
    popd || error_fn
    echo
fi

if [ "${DOVE_LINUX_FLATPAK}" == 1 ]; then
    if [[ "${DOVE_OS}" == 'osx' ]]; then
        ${DOVE_DOT_CLEAN} "${DOVE_LINUX_FLATPAK_OUTPUTS}" || error_fn
        echo
    fi

    pushd "${DOVE_LINUX_FLATPAK_OUTPUTS}" || error_fn
    echo
    echo_green_text "Creating ${DOVE_ARCHIVES}/dove-flatpak.zip..."
    ${DOVE_ZIP} "${DOVE_ARCHIVES}/dove-flatpak.zip" * -x 'unused/*' || error_fn
    echo
    popd || error_fn
    echo
fi

if [ "${DOVE_OSX}" == 1 ]; then
    if [[ "${DOVE_OS}" == 'osx' ]]; then
        ${DOVE_DOT_CLEAN} "${DOVE_OSX_OUTPUTS}" || error_fn
        echo
    fi

    pushd "${DOVE_OSX_OUTPUTS}" || error_fn
    echo
    echo_green_text "Creating ${DOVE_ARCHIVES}/dove-osx.zip..."
    ${DOVE_ZIP} "${DOVE_ARCHIVES}/dove-osx.zip" * -x 'Library/*' 'unused/*' || error_fn
    echo
    popd || error_fn
    echo
fi

if [ "${DOVE_OSX_INTEL}" == 1 ]; then
    if [[ "${DOVE_OS}" == 'osx' ]]; then
        ${DOVE_DOT_CLEAN} "${DOVE_OSX_INTEL_OUTPUTS}" || error_fn
        echo
    fi

    pushd "${DOVE_OSX_INTEL_OUTPUTS}" || error_fn
    echo
    echo_green_text "Creating ${DOVE_ARCHIVES}/dove-osx-intel.zip..."
    ${DOVE_ZIP} "${DOVE_ARCHIVES}/dove-osx-intel.zip" * -x 'Library/*' 'unused/*' || error_fn
    echo
    popd || error_fn
    echo
fi

if [ "${DOVE_WINDOWS}" == 1 ]; then
    if [[ "${DOVE_OS}" == 'osx' ]]; then
        ${DOVE_DOT_CLEAN} "${DOVE_WINDOWS_OUTPUTS}" || error_fn
        echo
    fi

    pushd "${DOVE_WINDOWS_OUTPUTS}" || error_fn
    echo
    echo_green_text "Creating ${DOVE_ARCHIVES}/dove-windows.zip..."
    ${DOVE_ZIP} "${DOVE_ARCHIVES}/dove-windows.zip" * -x 'unused/*' || error_fn
    echo
    popd || error_fn
    echo
fi
