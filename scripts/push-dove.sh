...
if [[ -z "${DOVE_FROM_PUSH+x}" ]]; then
    echo_red_text 'ERROR: Do not call push-dove.sh directly. Instead, use push.sh.' >&1
    exit 1
fi
...
readonly DOVE_PUSH_LINUX
readonly DOVE_PUSH_LINUX_FLATPAK
readonly DOVE_PUSH_OSX
readonly DOVE_PUSH_OSX_INTEL
readonly DOVE_PUSH_WINDOWS
