...
function push_file() {
  ...
  local -r s3_access_key=$("${DOVE_CAT}" "${DOVE_CEL_ARTIFACTS_S3_ACCESS_KEY_FILE}" | "${DOVE_XARGS}")
  local -r s3_bucket_name=$("${DOVE_CAT}" "${DOVE_CEL_ARTIFACTS_S3_BUCKET_NAME_FILE}" | "${DOVE_XARGS}")
  local -r s3_endpoint=$("${DOVE_CAT}" "${DOVE_CEL_ARTIFACTS_S3_ENDPOINT_FILE}" | "${DOVE_XARGS}")
  local -r s3_secret_key=$("${DOVE_CAT}" "${DOVE_CEL_ARTIFACTS_S3_SECRET_KEY_FILE}" | "${DOVE_XARGS}")

  if [[ "${s3_path}" == 'root' ]]; then
    local -r s3_target_path="s3://${s3_bucket_name}/${DOVE_CEL_S3_PROJECT}"
  else
    local -r s3_target_path="s3://${s3_bucket_name}/${DOVE_CEL_S3_PROJECT}/${s3_full_path}"
  fi
