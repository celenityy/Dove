...
# Remove lines containing [NO-LINUX], [OSX-ONLY], and [WINDOWS-ONLY]
grep -vE 'NO-LINUX|OSX-ONLY|WINDOWS-ONLY' "$DOVE_USER_PREF_CFG" > "$DOVE_LINUX_USER_PREF_CFG"
echo "Created $DOVE_LINUX_USER_PREF_CFG"

# Remove lines containing [ANDROID-ONLY], [NO-LINUX], [NO-MAIL], [OSX-ONLY], and [WINDOWS-ONLY]
grep -vE 'ANDROID-ONLY|NO-LINUX|NO-MAIL|OSX-ONLY|WINDOWS-ONLY' "$PHOENIX_UNIFIED_PREFS" > /tmp/dove/linux-temp1.js
echo "Created /tmp/dove/linux-temp1.js"

# Remove lines containing [ANDROID-ONLY], [NO-LINUX], [NO-MAIL], [OSX-ONLY], and [WINDOWS-ONLY]
grep -vE 'ANDROID-ONLY|NO-LINUX|NO-MAIL|OSX-ONLY|WINDOWS-ONLY' "$PHOENIX_EXTENDED_UNIFIED_PREFS" > /tmp/dove/linux-temp2.js
echo "Created /tmp/dove/linux-temp2.js"

# Remove lines containing [NO-LINUX], [OSX-ONLY], and [WINDOWS-ONLY]
grep -vE 'NO-LINUX|OSX-ONLY|WINDOWS-ONLY' "$DOVE_UNIFIED_PREFS" > /tmp/dove/linux-temp3.js
...
# Remove lines containing [LINUX-ONLY], [NO-OSX], and [WINDOWS-ONLY]
grep -vE 'LINUX-ONLY|NO-OSX|WINDOWS-ONLY' "$DOVE_BOOTSTRAP" > "$DOVE_OSX_BOOTSTRAP"
echo "Created $DOVE_OSX_BOOTSTRAP"

# Remove lines containing [ANDROID-ONLY], [LINUX-ONLY], [NO-MAIL], [NO-OSX], and [WINDOWS-ONLY]
grep -vE 'ANDROID-ONLY|LINUX-ONLY|NO-MAIL|NO-OSX|WINDOWS-ONLY' "$PHOENIX_UNIFIED_PREFS" > /tmp/dove/osx-temp1.js
echo "Created /tmp/dove/osx-temp1.js"

# Remove lines containing [ANDROID-ONLY], [LINUX-ONLY], [NO-MAIL], [NO-OSX], and [WINDOWS-ONLY]
grep -vE 'ANDROID-ONLY|LINUX-ONLY|NO-MAIL|NO-OSX|WINDOWS-ONLY' "$PHOENIX_EXTENDED_UNIFIED_PREFS" > /tmp/dove/osx-temp2.js
echo "Created /tmp/dove/osx-temp2.js"

# Remove lines containing [LINUX-ONLY], [NO-OSX], and [WINDOWS-ONLY]
grep -vE 'LINUX-ONLY|NO-OSX|WINDOWS-ONLY' "$DOVE_UNIFIED_PREFS" > /tmp/dove/osx-temp3.js
...
# Remove lines containing [LINUX-ONLY], [NO-WINDOWS], and [OSX-ONLY]
grep -vE 'LINUX-ONLY|NO-WINDOWS|OSX-ONLY' "$DOVE_BOOTSTRAP" > "$DOVE_WINDOWS_BOOTSTRAP"
echo "Created $DOVE_WINDOWS_BOOTSTRAP"

# Remove lines containing [ANDROID-ONLY], [LINUX-ONLY], [NO-MAIL], [NO-WINDOWS], and [OSX-ONLY]
grep -vE 'ANDROID-ONLY|LINUX-ONLY|NO-MAIL|NO-WINDOWS|OSX-ONLY' "$PHOENIX_UNIFIED_PREFS" > /tmp/dove/windows-temp1.js
echo "Created /tmp/dove/windows-temp1.js"

# Remove lines containing [ANDROID-ONLY], [LINUX-ONLY], [NO-MAIL], [NO-WINDOWS], and [OSX-ONLY]
grep -vE 'ANDROID-ONLY|LINUX-ONLY|NO-MAIL|NO-WINDOWS|OSX-ONLY' "$PHOENIX_EXTENDED_UNIFIED_PREFS" > /tmp/dove/windows-temp2.js
echo "Created /tmp/dove/windows-temp2.js"

# Remove lines containing [LINUX-ONLY], [NO-WINDOWS], and [OSX-ONLY]
grep -vE 'LINUX-ONLY|NO-WINDOWS|OSX-ONLY' "$DOVE_UNIFIED_PREFS" > /tmp/dove/windows-temp3.js
...
jq -s '.[0] * .[1]' /tmp/dove/temp4.json "$DOVE_UNIFIED_OSX_POLICIES" > "$DOVE_OSX_POLICIES_JSON"

jq -s '.[0] * .[1]' "$DOVE_POLICIES" "$PHOENIX_UNIFIED_WINDOWS_POLICIES" > /tmp/dove/temp5.json

jq -s '.[0] * .[1]' /tmp/dove/temp5.json "$DOVE_UNIFIED_WINDOWS_POLICIES" > "$DOVE_WINDOWS_POLICIES"
